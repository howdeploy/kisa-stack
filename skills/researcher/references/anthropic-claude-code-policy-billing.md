...
- Claude Code costs docs: для Pro/Max session cost estimate в `/usage` не является биллинговой суммой; usage включён в подписку.
- Help Center: если выставлен `ANTHROPIC_API_KEY`, Claude Code использует API key вместо подписки и usage идёт как API charges.
- Help Center: Pro/Max usage limits общие между Claude и Claude Code.
- Help Center: после исчерпания included usage можно включить extra usage, которое идёт по standard API rates.
...
Частый правильный вывод: вопрос не в том, «external tool» ли это сам по себе, а в том, идёт ли сценарий через subscription/OAuth или через API/commercial path.
...
- где остаётся серая зона интерпретации ToS
