...
Секция для тех, кто гоняет TTS через Hermes. В Claude Code / Codex скилл работает и без неё: правилам следуешь по SKILL.md, а `scripts/preflight.py` можно дергать вручную перед отправкой текста в ElevenLabs (модель задаётся env-переменными `HERMES_TTS_PROVIDER=elevenlabs` и `HERMES_TTS_MODEL_ID=...`).
